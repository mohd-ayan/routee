...
import { PristinePerfectScrollbarGeometry, PristinePerfectScrollbarPosition } from '@pristinebase/directives/pristine-perfect-scrollbar/pristine-perfect-scrollbar.interfaces';
import { PristineConfigService } from '@pristinebase/services/config.service';

@Directive({
    selector: '[pristinePerfectScrollbar]'
})
export class PristinePerfectScrollbarDirective implements OnInit, AfterViewInit, OnDestroy
{
     ...
     */
    @Input()
    set pristinePerfectScrollbarOptions(value)
    {
        // Merge the options
    ...
    get pristinePerfectScrollbarOptions(): any
    {
        // Return the options
     ...
     * @param {boolean | ""} value
     */
    @Input('pristinePerfectScrollbar')
    ...
    ngAfterViewInit(): void
    {
        ...
        if ( this.pristinePerfectScrollbarOptions.updateOnRouteChange )
        {
            this._router.events
                .pipe(
                    takeUntil(this._unsubscribeAll),
    ...
    _init(): void
    {
        ...
        this.ps = new PerfectScrollbar(this.elementRef.nativeElement, {
            ...this.pristinePerfectScrollbarOptions
        });
     ...
     * @param prefix
     */
    geometry(prefix: string = 'scroll'): PristinePerfectScrollbarGeometry
    {
        return new PristinePerfectScrollbarGeometry(
     ...
     * @param absolute
     */
    position(absolute: boolean = false): PristinePerfectScrollbarPosition
    {
        if ( !absolute && this.ps )
        {
            return new PristinePerfectScrollbarPosition(
                this.ps.reach.x || 0,
                this.ps.reach.y || 0
            );
        }
